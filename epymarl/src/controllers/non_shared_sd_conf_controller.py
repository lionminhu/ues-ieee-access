...
class NonSharedSdConfMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        return chosen_actions, agent_msg_outputs

    def forward(self, ep_batch, t, test_mode=False, use_msg=True):
        ...
        if use_msg:
            ...
            if self.share_sd_params:
                for agent_idx in range(self.n_agents):
                    ...
                    all_messages.append(_messages)
            else:
                for agent_idx in range(self.n_agents):
                    _messages, _, _ = self.models[agent_idx](ep_batch, t=t, agent_idx=agent_idx)
                    all_messages.append(_messages)
            all_messages = th.cat(all_messages, dim=-1)
            all_messages = all_messages.view(ep_batch.batch_size, self.n_agents, self.args.msg_len)
        else:
            all_messages = None

        return agent_outs, all_messages
