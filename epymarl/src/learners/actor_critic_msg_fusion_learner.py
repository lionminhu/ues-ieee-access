...
class ActorCriticMsgFusionLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        advantages, critic_train_stats = self.train_critic_sequential(self.critic, self.target_critic, batch, rewards,
                                                                      critic_mask)
        actions = actions[:, :-1]
        messages = messages[:, :-1, :, :self.args.msg_ext_len]
        ...
        # for idx in range(self.args.msg_len):
        messages = messages.unsqueeze(4).long()
        pi_taken = th.gather(msg_ext_outs, dim=4, index=messages).squeeze(4)
        log_pi_taken = th.log(pi_taken + 1e-10)
        entropy = -th.sum(msg_ext_outs * th.log(msg_ext_outs + 1e-10), dim=-1)
        ...
        pg_loss += -((advantages_msg * log_pi_taken + self.args.entropy_coef * entropy) * mask_msg).sum() / mask_msg.sum()
