...
class PPOMsgFusionLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        messages = batch["messages"][:, :-1, :, :self.args.msg_ext_len].unsqueeze(4).long()
        ...
        for t in range(batch.max_seq_length - 1):
            agent_outs, msg_ext_out, _ = self.old_mac.forward(batch, t=t, use_msg_sd=False)
        ...
        mask_msg = th.repeat_interleave(mask, self.args.msg_ext_len, dim=2)
        ...
        for k in range(self.args.epochs):
            ...
            for t in range(batch.max_seq_length - 1):
                agent_outs, msg_ext_out, _ = self.mac.forward(batch, t=t, use_msg_sd=False)
            ...
            advantages_msg = th.repeat_interleave(advantages, self.args.msg_ext_len, dim=2)
    ...
    def _update_targets(self):
        self.target_critic.load_state_dict(self.critic.state_dict(), is_target=True)
    ...
    def load_models(self, path):
        ...
        if not self.args.load_from_param_shared:
            # When transfer learning from shared -> unshared parameters
            # Don't load optimizer (not sure what correct behaviour of optimizers should be)
