...
from robotic_warehouse.warehouse import RewardType
...
class WarehouseCustom(gym.Env):
    ...
    def __init__(
        ...
        goals,
        add_rand_obst = False,
        rand_obst_pos_list = None,
        rand_obst_num = None,
        random_agent_pos = False,
        rand_num_agents = None,
        nonrand_agent_pos = None,
        ...
        terminate_on_deliv = True,
    ):
        self.obstacles = obstacles
        self.grid_size = obstacles.shape

        self.add_rand_obst = add_rand_obst
        if add_rand_obst:
            assert rand_obst_pos_list is not None
            self.rand_obst_pos_list = rand_obst_pos_list
            assert rand_obst_num is not None and rand_obst_num > 0
            self.rand_obst_num = rand_obst_num

        self.random_agent_pos = random_agent_pos
        if random_agent_pos:
            assert rand_num_agents is not None
            self.n_agents = rand_num_agents
            self.avail_agent_pos = [(y, x) for (y, x) in \
                product(range(self.grid_size[0]), range(self.grid_size[1])) if obstacles[y, x] == 0]
        else:
            assert nonrand_agent_pos is not None
            self.nonrand_agent_pos = nonrand_agent_pos
            self.n_agents = len(nonrand_agent_pos)

        self.shelf_spawns = shelves

        self.random_request = random_request
        if random_request:
            assert request_queue_size is not None
            self.request_queue_size = request_queue_size
        else:
            assert nonrand_req_shelves is not None
            self.nonrand_req_shelves = nonrand_req_shelves
        ...
        self.grid[_LAYER_OBSTACLES] = obstacles

        self.num_delivers = 0

        self.block_shelf_to_shelf = block_shelf_to_shelf
        self.terminate_on_deliv = terminate_on_deliv
    ...
    def _is_highway(self, y: int, x: int) -> bool:
        return (
            self.grid[_LAYER_OBSTACLES, y, x] == 0
            and (y, x) not in self.shelf_spawns
        )
    ...
    def _is_obstacle_or_wall(self, y, x):
        return (
            ...
            or bool(self.grid[_LAYER_OBSTACLES, y, x])
        )
    ...
    def reset(self):
        ...
        # make the shelfs
        self.shelfs = [Shelf(y, x) for y, x in self.shelf_spawns]

        self.grid[_LAYER_OBSTACLES] = self.obstacles
        if self.add_rand_obst:
            new_obst_pos_idx = np.random.choice(range(len(self.rand_obst_pos_list)),
                self.rand_obst_num, replace=False)
            new_obst_pos = [self.rand_obst_pos_list[i] for i in new_obst_pos_idx]
            for y, x in new_obst_pos:
                self.grid[_LAYER_OBSTACLES, y, x] = 1

        if self.random_agent_pos:
            avail_agent_pos = self.avail_agent_pos
            if self.add_rand_obst:
                avail_agent_pos = list(set(avail_agent_pos) - set(new_obst_pos))
            agent_loc_idx = np.random.choice(range(len(avail_agent_pos)),
                self.n_agents, replace=False)
            agent_locs = [avail_agent_pos[i] for i in agent_loc_idx]
        else:
            agent_locs = self.nonrand_agent_pos
        ...
        if self.random_request:
            self.request_queue = list(np.random.choice(self.shelfs, self.request_queue_size,
                replace=False))
        else:
            self.request_queue = [self._shelf_at_loc(y, x) for (y, x) in self.nonrand_req_shelves]
            assert None not in self.request_queue

        self.num_delivers = 0

        return tuple([self._make_obs(agent) for agent in self.agents])

    def _force_request(self, desired_requests):
        # self.reset()
        self.request_queue = [self._shelf_at_loc(y, x) for (y, x) in desired_requests]
        assert None not in self.request_queue
    ...
    def step(
        ...
        for agent in self.agents:
            ...
            if (
                (
                    agent.carrying_shelf or
                    (self.block_shelf_to_shelf and self.grid[_LAYER_SHELFS, start[0], start[1]])
                )
                and start != target
                # check whether a shelf is in the way
                and self.grid[_LAYER_SHELFS, target[0], target[1]]
                # check whether the shelf in the way is carried by another agent that
                # may move it out of the way
                and not (
                ...
                # also block agent moving from block to block if necessary
        ...
        for y, x in self.goals:
            ...
            if self.reward_type == RewardType.GLOBAL:
                ...
                rewards[agent_id:] += self.reward_share_wgt
            elif self.reward_type == RewardType.TWO_STAGE:
                agent_id = self.grid[_LAYER_AGENTS, y, x]
                self.agents[agent_id - 1].has_delivered = True
                rewards[agent_id - 1] += 0.5
                rewards[:agent_id - 1] += self.reward_share_wgt * 0.5
                rewards[agent_id:] += self.reward_share_wgt * 0.5

        if len(shelf_delivered) > 0:
            if not self.terminate_on_deliv:
                new_requests = np.random.choice(
                    list(set(self.shelfs) - set(self.request_queue)),
                    size = len(shelf_delivered), replace = False
                )
                for idx, old_shelf in enumerate(shelf_delivered):
                    self.request_queue[self.request_queue.index(old_shelf)] = new_requests[idx]
            else:
                self.request_queue = list(set(self.request_queue) - set(shelf_delivered))

        self._cur_steps += 1

        if ((self.terminate_on_deliv and len(self.request_queue) == 0)
    ...
    def draw(self):
        ...
        canvas_height = (
            width_config["outline"] + self.grid_size[0] *
            (width_config["outline"] + width_config["tile_width"])
        )
        canvas_width = (
            width_config["outline"] + self.grid_size[1] *
            (width_config["outline"] + width_config["tile_width"])
        )
        ...
        for y in range(self.grid_size[0]):
            for x in range(self.grid_size[1]):
                tile_top_y = (
                    width_config["outline"] +
                    y * (width_config["outline"] + width_config["tile_width"])
                )
                tile_left_x = (
                    width_config["outline"] +
                    x * (width_config["outline"] + width_config["tile_width"])
                )

                # draw goal tiles
                if (y, x) in self.goals:
                    top_left = (tile_left_x, tile_top_y)
                    bottom_right = (
                        tile_left_x + width_config["tile_width"] - 1,
                        tile_top_y + width_config["tile_width"] - 1
                    )
                ...
                if shelf_id != 0:
                    ...
                    top_left = (tile_left_x + pad, tile_top_y + pad)
                    bottom_right = (
                        tile_left_x + pad + width_config["shelf_width"] - 1,
                        tile_top_y + pad + width_config["shelf_width"] - 1
                    )
                ...
                if agent_id != 0:
                    ...
                    center = (
                        ...
                        tile_top_y + width_config["tile_width"] // 2
                    )
        ...
        for agent in self.agents:
            bottom_y = (agent.y + 1) * (width_config["outline"] + width_config["tile_width"])
            # left_x = width_config["outline"] + (agent.x + 1) * (width_config["outline"] + \
            #     width_config["tile_width"])
            left_x = agent.x * (width_config["outline"] + width_config["tile_width"])
