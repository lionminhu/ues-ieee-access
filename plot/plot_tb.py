...
def draw_plots(run_ids, result_png_path, config, parse_func=default_parse):
    ...
    for model_idx in range(len(run_ids)):
        model_name = list(run_ids.keys())[model_idx]

        # x values to interpolate to
        inter_xs = list(range(min_inter_x, max_inter_x, interval))

        # Ignore model with no file provided
        if len(run_ids[model_name]) == 0:
            continue

        # Get mean and std across files of the model
        # print(f"*** draw_plots 1: {y_label}, {len(inter_xs)}")
        inter_xs, mean_y, std_y = get_curve(
            run_ids[model_name],
            interval,
            min_inter_x,
            max_inter_x,
        ...
        # Draw mean
        ax.plot(
            inter_xs, mean_y, color=colors[model_idx],
        ...
        ax.fill_between(
            inter_xs, lower_y, upper_y, color=colors[model_idx], alpha=0.3
        )
...
def check_best_mean_perf(config, log_dir, exp_note, git_commit, file_names,
                         parse_func=default_parse):
    log_path = os.path.join(log_dir, "check_best_mean_perf_config.log")
    with open(log_path, "w") as out_file:
        out_file.write(f"exp note: {exp_note}\n")
        out_file.write(f"git commit: {git_commit}\n")
        out_file.write("config:\n")
        out_file.write(json.dumps(config, indent=2))
        out_file.write("\n")
        out_file.write("-----------------\n")

    min_inter_x = config["min_updates"]
    max_inter_x = config["max_updates"]
    interval = config["interval"]
    
    for model_name, perf_log_paths in file_names.items():
        if len(perf_log_paths) == 0:
            continue

        _, mean_ys, std_ys = get_curve(perf_log_paths, interval, min_inter_x,
                                       max_inter_x, parse_func=parse_func)
        best_idx = mean_ys.argmax()
        best_y_mean = mean_ys[best_idx]
        best_y_std = std_ys[best_idx]

        msg = f"{model_name}: {float(best_y_mean):.5f} +/- {float(best_y_std):.5f}"
        with open(log_path, "a") as out_file:
            out_file.write(msg + "\n")
        print(msg)


def check_mean_best_perf(run_ids, log_dir, config, parse_func=default_parse):
    log_path = os.path.join(log_dir, "check_mean_best_perf_config.log")
    with open(log_path, "w") as out_file:
        pass
        # out_file.write(f"exp note: {exp_note}\n")
        # out_file.write(f"git commit: {git_commit}\n")
    ...
    min_inter_x = config["xaxis_bound"][0]
    max_inter_x = config["xaxis_bound"][1]
    interval = config["interval"]

    for model_name, perf_log_paths in run_ids.items():
        if len(perf_log_paths) == 0:
            continue

        best_ys = []
        for perf_log_path in perf_log_paths:
            _, ys, _ = get_curve(
                [perf_log_path],
                interval,
                min_inter_x,
        ...
        mean_best_y = np.mean(best_ys)
        std_best_y = np.std(best_ys)

        msg = '{}: {:.5f} +/- {:.5f}'.format(model_name, mean_best_y, std_best_y)
        with open(log_path, "a") as out_file:
            out_file.write(msg + '\n')
        print(msg)